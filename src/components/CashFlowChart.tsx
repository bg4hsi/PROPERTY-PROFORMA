...
const money2 = (value: number) => new Intl.NumberFormat("zh-CN", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(value || 0);
const colors = {
  ...
  return <section className="space-y-4" id="cash-flow">
    <div className="card overflow-hidden">
      ...
      <div className="overflow-x-auto px-3 pb-4"><svg viewBox={`0 0 ${width} ${height}`} className="min-w-[900px] w-full" role="img" aria-label="项目全周期现金流量图" onMouseLeave={()=>setHoveredMonth(null)}>
        ...
        {hovered && <foreignObject x={tooltipX} y={top+14} width={tooltipWidth} height="280" pointerEvents="none" data-tooltip-side={tooltipSide}><div className="rounded-xl border border-slate-200 bg-white p-4 shadow-xl"><div className="mb-2 text-lg font-bold text-slate-900">第{hovered.month}个月</div><div className="space-y-1.5 text-sm font-medium tabular-nums"><div style={{color:colors.salesBar}}>当月新增销售：{money2(hovered.monthlySales)} 万元</div><div style={{color:colors.collectionBar}}>当月回款/经营净流入：{money2(hovered.monthlyCollection)} 万元</div><div className="pl-3 text-xs text-emerald-600">其中经营净现金流：{money2(hovered.monthlyOperatingCashFlow)} 万元</div><div style={{color:colors.outflowBar}}>当月项目支出：{money2(hovered.monthlyOutflow)} 万元</div><div className="my-2 border-t border-slate-100"/><div style={{color:colors.sales}}>累计销售额：{money2(hovered.cumulativeSales)} 万元</div><div style={{color:colors.collection}}>累计回款及经营净流入：{money2(hovered.cumulativeCollection)} 万元</div><div style={{color:colors.outflow}}>累计支出：{money2(hovered.cumulativeOutflow)} 万元</div><div style={{color:colors.net}}>累计净现金流：{money2(hovered.cumulativeNetCashFlow)} 万元</div></div></div></foreignObject>}
