...
export function CollectionLogicPanel({ rows, cutoffMonth, setCutoffMonth, updateRow }: { rows: CalculatedRow[]; cutoffMonth: number; setCutoffMonth: (month: number) => void; updateRow: (id: string, patch: Partial<AssetRow>) => void }) {
  ...
  const fields: Array<[keyof CollectionLogic, string, boolean]> = [
    ["firstSaleMonth", "首售（月）", false], ["totalUnits", "总套数", false], ["monthlyAbsorptionUnits", "月去化（套）", false],
  ...
  const monthlySalesAmount = (row: CalculatedRow) => { const logic = defaultCollectionLogic(row); return logic.totalUnits > 0 ? row.revenue * logic.monthlyAbsorptionUnits / logic.totalUnits : 0; };
  ...
  const logicTotals = salesRows.reduce<Record<keyof CollectionLogic, number>>((sum,row)=>{
    const logic=defaultCollectionLogic(row);
    for(const [key] of fields) sum[key]+=logic[key];
    return sum;
  ...
  return <section className="card overflow-hidden" id="collection-logic">
    <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-200 px-5 py-4"><div><h2 className="font-semibold text-slate-900">销售与回款逻辑设置</h2><p className="mt-1 text-xs text-slate-500">支持期房/现房：按每月销售批次计算首付、过程回款与交付后尾款；交付月统一取项目基础设置</p></div><span className="rounded-full bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700">截止第 {cutoffMonth} 月成果</span></div>
