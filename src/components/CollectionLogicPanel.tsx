"use client";
import { calculateCollectionSchedule, defaultCollectionLogic } from "@/lib/calculationEngine";
...
export function CollectionLogicPanel({ rows, cutoffMonth, setCutoffMonth, updateRow }: { rows: CalculatedRow[]; cutoffMonth: number; setCutoffMonth: (month: number) => void; updateRow: (id: string, patch: Partial<AssetRow>) => void }) {
  ...
  const rowResult = (row: CalculatedRow) => {
    ...
    return { area: logic.totalUnits ? row.saleArea * units / logic.totalUnits : 0, sales: projection?.cumulativeSales[index] || 0, collection: projection?.cumulativeCollection[index] || 0 };
  };
  const totals = salesRows.reduce((sum, row) => { const value = rowResult(row); return { area: sum.area + value.area, sales: sum.sales + value.sales, collection: sum.collection + value.collection }; }, { area: 0, sales: 0, collection: 0 });
