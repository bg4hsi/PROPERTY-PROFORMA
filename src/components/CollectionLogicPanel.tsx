...
export function CollectionLogicPanel({ rows, project, cutoffMonth, setCutoffMonth, updateRow }: { rows: CalculatedRow[]; project: ProjectInfo; cutoffMonth: number; setCutoffMonth: (month: number) => void; updateRow: (id: string, patch: Partial<AssetRow>) => void }) {
  ...
  const editableFields: Array<[keyof CollectionLogic, string]> = [
    ["firstSaleMonth", "首售（月）"], ["monthlyAbsorptionUnits", "月去化（套）"]
  ];
  const referenceFields: Array<[keyof CollectionLogic, string, boolean]> = [
  ...
  const monthlySalesAmount = (row: CalculatedRow) => { const logic = defaultCollectionLogic(row, project); return logic.totalUnits > 0 ? row.revenue * logic.monthlyAbsorptionUnits / logic.totalUnits : 0; };
